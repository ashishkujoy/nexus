...
export const fetchBatchesAssigned = async (userId: number, isRoot: boolean) => {
  ...
  const rows = await (isRoot ? sql`SELECT * FROM batches` : sql`
    ...
    JOIN batches b ON ma.mentor_id = ${userId} AND ma.batch_id = b.id
  ...
  return rows.map((row: any) => ({
    ...
    permissions: isRoot ? ({ recordObservation: true, recordFeedback: true, programManager: true }) : (row.permissions),
    root: isRoot
  }));
}